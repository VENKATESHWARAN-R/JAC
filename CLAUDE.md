...
- **SDK control plane — shared runtime mutations across surfaces (D49, 2026-05-31).** Every runtime mutation a surface triggers — switch model/profile, enable/disable/reload MCP, reload skills, refresh toolsets (mode change) — lives once on `SessionController` (`runtime/control.py`), wrapping a live `SessionRuntime` and mutating it in place. CLI slash handlers and web endpoints are thin adapters that call its verbs and render the returned `ControlResult` their own way; **no rebuild/toggle logic is duplicated per surface** (this killed the web's forked `_rebuild` and a bug where the web MCP toggle wrote a file but never rebuilt Gru). This is the prerequisite that makes ACP (D45) a thin adapter. Layering: engine (`build_session_runtime`) → control plane (`SessionController`) → surface adapters (CLI / web / A2A / future ACP). **Also (D49):** the inbound A2A server is started **only** via `jac a2a serve` — `/a2a serve|stop|status|token` were removed from the REPL (mirroring `jac web serve`); `/a2a peers` + `/a2a peer add|remove` (outbound peer config) stay.
...
- **ACP — editor surface (D45, condition-gated).** `ACPCapability` wrapping the Python [ACP SDK](https://agentclientprotocol.com) (`agent-client-protocol`). ACP is the LSP analogue for coding agents — one spec, any compliant editor. VS Code / Zed / JetBrains extensions become generic ACP clients; JAC writes the server once. **Ship conditions (re-assessed 2026-05-31):** (1) ACP remote HTTP/WebSocket transport stabilises — *still WIP, the remaining gate*; (2) a major editor ships an ACP client — **now met** (Zed native, JetBrains shipping). Per the 2026-05-31 decision ACP stays **design-only** until (1) clears, even though stdio (what editors actually spawn) is already stable. The SDK-first control plane (D49) makes ACP a thin Layer-3 adapter when unblocked. **Drop-in design:** [`docs/design/acp-surface.md`](docs/design/acp-surface.md); locked in D45.
