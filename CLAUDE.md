...
- **Web surface — local-first control panel + chat (D48). Slice 1 shipped.** A third surface alongside CLI and A2A: a Starlette + HTMX browser UI under `src/jac/web/` (mirrors `src/jac/cli/`), reached via `jac web serve`. It is a *renderer + management API* over the same `jac.sdk` engine (`SessionDriver` / `EventBus` / capabilities) — **not** a new runtime mode (refuse any change that makes it one). **Local-first, single-user by charter: binds `127.0.0.1`, no accounts, never multi-tenant.** The loopback boundary *is* the security model; a non-loopback `--host` is allowed but warns loudly because the settings panel reads and writes API keys in the clear. Session scope follows the launch directory via `paths.project_state_root()` — **project-only in v1** (running in project A shows project A's sessions); cross-project browsing is deliberately deferred. Slices: 1 = config/session control panel (profiles, providers/keys, secrets, sessions — done), 2 = streaming chat + HITL over SSE/WebSocket, 3 = minion dashboard. Design: [`docs/design/web-surface.md`](docs/design/web-surface.md).
