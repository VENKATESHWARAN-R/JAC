...
Phase A (foundation, shipped v0.3.0) handles levers 3 and 4 — pure plumbing, biggest immediate cost win. Phase B (shipped v0.3.0) introduces lever 1 (sub-agents). Phase D (shipped v0.4.0) adds the skill loader. Phase E (shipped v0.5.0) adds parallel fan-out (`spawn_sub_agents`) and bidirectional sub-agent ↔ main-agent comms (D41, on by default). Lever 2 (tier-aware) is partially shipped via D22 and extended in Phase B with the HITL-approved tier on spawn. Lever 5 (deterministic hooks) was considered and dropped — see D37. Phase F (MCP loader + tool search, shipped 2026-05-29) adds the ecosystem surface — external MCP servers wired in as deferred-loaded, HITL-gated toolsets (D46, D28). Phase ordering after the 2026-05-27 external review: **F=MCP loader (done), G=Plan Mode (D23), H=A2A 4.e + broader tests**. Sandboxing for v2 uses `pydantic-monty` directly (D43); MCP-side programmatic tool calling (code mode) is part of that same v2 deferral, not Phase F. Evaluation via Logfire span replay is a Phase 7 stream (D44). Editor surface for v2 uses ACP (D45, condition-gated). See [`design/cost-efficient-orchestration.md`](design/cost-efficient-orchestration.md) for the full Phase A-H design and [`progress-roadmap.md`](progress-roadmap.md) for Harness reuse-vs-build decisions and the full ACP design.
...
| **Modes (Plan / Accept-Edits / YOLO)** | `jac.runtime.modes` policy | `approval_override` knob (Plan auto-denies gated calls; Accept-Edits auto-allows write/edit) + reserved `filter_capabilities`. Shipped v0.7.0 (Plan + Accept-Edits). Plan realises "read-only" via approval auto-deny so reads stay live; `plan`→`tasks` rename + `write_plan` deferred. YOLO knob built but **not exposed** — still v2 with `pydantic-monty` sandboxing (D43). (D23/D29/D43) |
...
| D11 | **Workspace layout:** user workspace at `~/.jac/`, project workspace at `<root>/.agents/`. A folder is a **project** when it has `.git` **or** `.agents/` at/above CWD (`paths.project_root()`; `.agents/` is the explicit opt-in for non-git folders, created by `jac init` / `bootstrap.init_project_workspace`). **Loose mode** (no project): state writers (sessions, `usage.jsonl`, tool-result cache, A2A) fall back to `~/.jac/` via `paths.project_state_root()` rather than scattering `.agents/` into the CWD; overlay resources (config/memory/prompts/skills) have no project layer and are skipped. `find_project_root()` is the *working* root (CWD fallback) for tools only. Sessions live at state-root scope. `AGENTS.md` at repo root (community convention) and `~/.jac/AGENTS.md`; both auto-loaded into instructions. JAC never writes to `AGENTS.md`. |
...
| D14 | **Memory write path (2×2):** `remember(reason, content, category, scope)` writes to `~/.jac/memory.md` (`scope="user"`) or `<repo>/.agents/memory.md` (`scope="project"`). `forget(reason, content, scope)` removes by exact match. `scope` is required; `scope="project"` outside a project (`.git`/`.agents`) raises. Audit comment per entry: `<!-- jac: <timestamp> session: <id> -->`. Soft ~25-entry size warning. The same tools are exposed directly as the `/remember` and `/forget` **slash commands** for user-driven edits (no model call; typing the command is the approval, bypassing the agent HITL panel). |
...
| D20 | **Token-aware history compaction (supersedes Phase 2b).** Budget resolved via `resolve_context_budget()` — session override (`/context`) → per-model `compaction.model_context_tokens` → `compaction.max_context_tokens` (default **256k** since v0.7.0, hard ceiling **512k**). `compaction.strategy` (v0.7.0): `auto` (warn 60% / summarize 70% via `small` tier / refuse 85%), `sliding` (drop oldest at 70%, never refuses, red overflow marker, no model call), or `manual` (`/compact` only). `/compact` forces a summarizing compaction in any strategy. Original slices preserved under `<session>/compacted/<n>.json` in every strategy. |
...
| D23 | **Plan Mode — shipped v0.7.0 (2026-05-30), with Accept-Edits.** As-built in `jac.runtime.modes`: a session-scoped policy with an `approval_override(tool_name)` knob (the runtime knob) + a reserved `filter_capabilities()`. **Plan Mode realises its "read-only subset" via approval auto-deny, not literal capability removal** — the filesystem toolset bundles `read_file` with `write_file`, so denying every *gated* call at the approval handler keeps reads live while blocking all mutations; the model is steered by an overridable prompt addendum (`gru_plan_mode.md`) + a guiding deny message. **Accept-Edits** (added in the same release) auto-approves `write_file`/`edit_file` only. `/mode` switches and rebuilds Gru (`RefreshToolsets`) so the addendum re-applies. **Deviations from the original sketch:** the bundled `plan`→`tasks` rename and a dedicated `write_plan` tool were **deferred** — pure churn (incl. a `plan.json`→`tasks.json` migration) with no user value; Plan Mode reuses the existing `plan`/`update_plan` checklist. YOLO's `approval_override` seam is built but **not exposed** (still v2 per D43). |
...
| D36 | **Task packet schema.** `SubAgentTaskPacket` Pydantic model (Phase B): `objective: str` (req), `success_criteria: list[str]` (opt), `relevant_paths: list[str]` (opt), `forbidden_actions: list[str]` (opt), `expected_output: str` (opt — free-text spec), `allowed_tools: list[str] \| None` (opt — **enforced at the Agent layer since the 2026-05-30 review Phase 3**: `_run_sub_agent` attaches a `PrepareTools` filter restricting the worker to the named tools plus an always-allowed control plane `{read_file, ask_main_agent}`; `None` → unfiltered), `max_turns: int = 10`. Supersedes D1 fields list. |
...
| D48 | **Web UI is a third surface — local-first & single-user (2026-05-31).** A browser control panel + (later) chat under `src/jac/web/`, served by `jac web serve`. **Stack:** Starlette + Jinja2 + HTMX, SSE for the event stream + WebSocket for HITL replies — **zero new dependencies** (all transitive via fasta2a / pydantic-ai). It is a *renderer + management API* over the same `SessionDriver` / `EventBus` / capability stack the CLI uses (`jac.sdk`), **not** a new runtime mode. **Charter: single-user, local-first — binds `127.0.0.1`, no accounts, never multi-tenant.** The loopback boundary *is* the security model; a non-loopback `--host` is allowed but warns loudly (the settings panel reads/writes API keys in the clear). **Session scope = the launch directory's `paths.project_state_root()`** — project-only in v1 (server in project A shows project A's sessions; loose dir shows the global `~/.jac` pool); cross-project browsing deferred (a session references its project's files; resuming it elsewhere is ambiguous). **Slices:** 1 = control panel (profiles / providers-keys / secrets / sessions, shipped), 2 = streaming chat + HITL, 3 = minion dashboard. **Frameworks rejected:** FastAPI (extra dep for a typed REST surface a local panel doesn't need), Chainlit / NiceGUI (own the page, fight the settings panel + theming), Reflex (heavy full-stack — revisit only if Slice 3's dashboard graduates). Full design: [`design/web-surface.md`](design/web-surface.md). |
