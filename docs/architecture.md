...
| D30 | **A2A subsystem internal layout (Phase 4, 2026-05-24).** `capabilities/a2a/` is structured as a subfolder rather than a single module — its responsibilities (server lifecycle, guest-Gru construction, auth middleware, agent-card generation, persistent storage, audit log, outbound tools, pluggable auth strategies) are too varied to share one file. Split: `__init__.py` exposes `A2ACapability` (public surface — registers outbound tools with Gru, owns server lifecycle handles + per-peer auth-strategy cache); `server.py` wraps `fasta2a.pydantic_ai.agent_to_a2a()` on a background asyncio task with clean shutdown; `guest.py` builds the guest Gru with the narrowed toolset + project context (no session memory, no minions); `auth.py` is the Starlette `BearerAuthMiddleware` + `generate_token()` (via `secrets.token_urlsafe(32)`) — *inbound* auth only; `card.py` auto-generates the `AgentCard` TypedDict from the active profile, including `securitySchemes` (omitted if `--unsafe`); `storage.py` is `JacFileStorage(fasta2a.Storage)` persisting tasks + contexts as JSON under `<project>/.agents/a2a/` with configurable retention; `audit.py` writes `inbound.jsonl` and runs context retention cleanup on server start + 1-hour timer; `client.py` is the outbound tools (`a2a_call` + `a2a_discover`) thin-wrapping `fasta2a.client.A2AClient` with auth-strategy-injected `httpx`; `auth_strategies.py` (D31) hosts the pluggable outbound auth strategies — `BearerStrategy`, `ApiKeyStrategy`, `OAuth2ClientCredentialsStrategy`, all behind an `AuthStrategy` Protocol so new strategies (`OidcStrategy`, `GcpIdTokenStrategy`) drop in without touching the call path. The CLI surface stays where every other capability puts it: `cli/slash/a2a.py` for slash handlers (`/a2a serve|stop|status|token|peers|peer add|peer remove`) and `cli/a2a.py` for the headless `jac a2a serve` typer command — both delegate into the same `A2ACapability` methods (no duplicate logic, mirroring the profiles / keys pattern). This precedent matches `capabilities/modes/` which is already structured as a subfolder for the same reason. |
| D31 | **Outbound A2A auth is a pluggable strategy per peer; secrets never cross the agent boundary (Phase 4.c, 2026-05-24).** PR1+PR2 shipped bearer-only outbound auth. That works for JAC↔JAC with a pre-shared token but blocks Azure / GCP / OIDC peers and forces the operator to hand-paste rotating tokens. D31 generalizes: each peer's `auth:` block in `a2a.peers.<name>` is a **discriminated union** (`bearer` / `api_key` / `oauth2_client_credentials`, with `oidc` / `gcp_id_token` to follow). The legacy `token: <str>` shorthand promotes to `auth: {type: bearer, token: <str>}` via a pydantic `model_validator(mode="before")` — no migration needed for current configs. **Strategy dispatch** lives in `capabilities/a2a/auth_strategies.py` behind an `AuthStrategy` Protocol (`async def headers_for() -> dict[str, str]`); `make_strategy(auth)` maps a config block to its strategy. **Token caching** for stateful strategies (OAuth2 access tokens) lives on the strategy instance, which the `A2ACapability` caches per peer via `id(peer.auth)` — instance-identity indexing means `/profile` swaps and slash-add operations naturally invalidate (new instance → new id → new strategy → fresh token fetch). Cache is per-session, in-memory, lazy-refresh; dies with the REPL. **Secrets storage** has two distinct surfaces by design: (a) *stable peers* live in profile YAML with `${ENV_VAR}` references resolved through `jac.secrets` (keyring/dotenv/env-only) — operator manages via `jac profiles edit`; (b) *ephemeral peers* (local dev, restart-rotating JAC↔JAC) are added via a new `/a2a peer add NAME URL ...` slash that prompts for secrets via `getpass.getpass()` and stores the `A2APeerConfig` in `A2ACapability.session_peers` — in-process only, never on disk, never in messages.json. Session peers override profile peers on name collision (loud warning; `/a2a peers` shows the shadowed entry greyed-out; `/a2a peer remove NAME` reverts). **Privacy guarantee**: Gru only ever passes a peer *name* in tool args (`a2a_call(peer_or_url="backend", ...)`) — the strategy resolution and credential I/O happen *inside* JAC code after the agent loop returns the tool call. The LLM provider never sees a secret regardless of which surface it came from. Discovery integration (validating that the peer's `securitySchemes` matches our configured `auth.type`) is *not* automatic per-call (cost) — operator can run `a2a_discover` explicitly to inspect. **OIDC + GCP ID tokens deferred to Phase 4.d** — they need a heavier dep (google-auth for GCP) and an additional discovery roundtrip pattern. The Protocol shape means they ship without touching `a2a_call`. |
