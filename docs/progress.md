...
> **Just Another Companion/CLI** · **Updated:** 2026-05-31 · keep this in sync as work lands.
...
- **Shipped in v0.9.0 (2026-05-31): the web surface (D48).** Slices 1–3, then the **redesign (R0–R5, 2026-05-31)** under `src/jac/web/` (`jac web serve`): a chat-first **light full-bleed Console** plus the full **Control Panel** — profiles, keys, scope/precedence-aware **config**, MCP, A2A, skills, context/prompts, providers, memory, dashboard/doctor — opened as htmx drawers over the live chat, with a top-bar **model/profile switcher** (mid-session rebuild + rollback guard) and a HITL-disconnect failsafe. New module `workspace/config_io.py` (scope-aware config writes + per-field precedence). `just check` green (768 tests). Design: [`design/web-surface.md`](design/web-surface.md) (the redesign is now folded in — single SSOT); surface/engine layering in [`architecture.md`](architecture.md) §1.5; user guide [`user-guide/web-ui.md`](user-guide/web-ui.md). Remaining: visual polish + a narrow-width responsive breakpoint. **Next up:** Phase H (A2A 4.e + broader tests). See the "Web surface" section below. **Phase G — Plan Mode (D23) landed 2026-05-30 in v0.7.0** alongside Accept-Edits mode + a compaction-control pass. **Phase F (MCP loader + tool search) landed 2026-05-29.** Also queued: Phase H (A2A 4.e + broader tests). YOLO mode is still v2 (needs `pydantic-monty` sandboxing per D43).
- **SDK control plane (D49) — released in v0.9.0, 2026-05-31 (`just check` green, 768 tests).** Extracted `SessionController` (`runtime/control.py`) — the surface-agnostic Layer 2 that owns every runtime mutation (switch model/profile, `refresh_toolsets`, `set_mcp_enabled`, `reload_mcp`, `reload_skills`). CLI slash handlers and web endpoints are now thin adapters over its verbs: removed the web's hand-copied `_rebuild` and the REPL's `_rebuild_gru`, deleted the `RebuildGru`/`RefreshToolsets` slash-result types, and **fixed a web bug** where toggling MCP wrote the file but never rebuilt Gru (no effect until restart) — the web now reloads the live catalog after any control-panel MCP/skill write, plus a new "Use in chat" skill action. `SessionRuntime` gained a `profile_name` field (single source of truth). **Also removed the inbound A2A server lifecycle from the REPL** — `/a2a serve|stop|status|token` are gone; start it with `jac a2a serve` (mirroring `jac web serve`); `/a2a peers` + `/a2a peer add|remove` stay. This is the SDK-first prerequisite that makes the ACP editor surface (D45) a thin adapter; see [`design/acp-surface.md`](design/acp-surface.md).
...
- **Released:** v0.3.0 (Phases A + B, 2026-05-27) · v0.4.0 (Phase D skill loader, 2026-05-27) · v0.5.0 (Phase E parallel + bidirectional, 2026-05-28) · v0.6.0 (workspace loose-mode, session management, memory slash commands, minion theme, 2026-05-29) · v0.7.0 (Plan + Accept-Edits modes, compaction strategies + `/compact` + `/context`, 256k/512k budget, 2026-05-30) · v0.8.0 (end-stage review remediation R1–R20: sub-agent allowlist enforcement, suspend/resume comms, the `SessionDriver`/`jac.sdk` expandability unlock, 2026-05-30) · **v0.9.0 (local-first web UI `jac web serve` — chat Console + full Control Panel + activity dashboard (D48) and the R0–R5 redesign; plus the SDK control plane `SessionController` (D49) that makes every surface a thin adapter over one engine; the inbound A2A server now starts only via `jac a2a serve`, 2026-05-31).**
...
| **Web surface (D48) — Slices 1–3 + redesign (R0–R5)** | ✅ Complete (v0.9.0) — panel + chat + dashboard (2026-05-31), then redesigned (R0–R5) | `src/jac/web/` (Starlette + HTMX + SSE, zero new deps), `jac web serve`. Local-first, single-user, loopback-bound. Slices 1–3 built the engine seam (shared `build_session_runtime`, SSE+HITL, activity dashboard). **Redesign:** chat-first **light full-bleed Console** + a full **Control Panel** — profiles, keys, scope/precedence-aware **config**, MCP, A2A, skills, context/prompts, providers, memory, dashboard/doctor — as htmx drawers over the live chat; top-bar **model/profile switcher** (mid-session rebuild + rollback guard); HITL-disconnect failsafe; new `workspace/config_io.py`. Renderer over `jac.sdk` — **not** a runtime mode. Remaining: visual polish + a narrow-width responsive breakpoint. Design: [`design/web-surface.md`](design/web-surface.md) (charter + redesign, single SSOT). |
...
- [x] AgentCard integration: loaded skills appended to A2A `skills:` list as `id: jac-skill-<name>`; generic skill remains as base advertisement. `skills_getter` on `A2ACapability` is a callable so `/skill reload` between server restarts is reflected on the next `jac a2a serve`.
...
**Slice 3 (done):** live activity sidebar on the chat page. `web/chat.py` tracks file-mutating tool calls (`write_file`/`edit_file`, minions included) and exposes a `dashboard()` snapshot — token/cost meter (session in/out/total, cache %, project total, budget %), minion cards, and the files-changed list. `GET /chat/status` serves it; `static/chat.js` polls (faster while a turn is active) and renders the sidebar, reusing the same registries the CLI's `/spawns` + `/tokens` read. Live-verified: `/chat/status` reflects real token usage after a turn.

**Chat UX pass (done, 2026-05-31):** three rough edges from live use, fixed.
- **HITL prompts no longer clutter the transcript.** Approvals/clarifies render one-at-a-time in a **pinned bar** above the input (`#hitl-bar`) with a `+N more waiting` queue counter, so concurrent approvals from a parallel `spawn_sub_agents` batch can't be missed or buried in scroll-back; only a one-line `notice` is left behind once resolved. Tool args show as labelled rows (long/structured values in a scrollable `<pre>`) instead of one raw JSON blob.
- **Minion cards now update for non-parked workers.** The dashboard's `active` list was sourced only from `_pending_spawns`, which holds *suspended* (bidirectional) workers — a parallel batch runs to completion without ever parking, so it never appeared. `WebChatManager` now also records `running` workers from the `SubAgentSpawned`/`SubAgentCompleted` bus lifecycle (cleared on attach), merged with the `waiting` `_pending_spawns` entries by `spawn_id`; `chat.js` polls immediately on those events so the card appears instantly.
- **Assistant replies render markdown.** Output was previously plain-`esc()`'d text. `chat.js` gained a small escape-first `renderMarkdown` (vendored, not a CDN dep — local-first; escapes before tokenizing so model output can't inject markup) covering headings, bold/italic/strike, inline + fenced code, lists, blockquotes, links, hr.

**Chat-centric layout (done, 2026-05-31):** restructured the web shell from a settings-panel-with-chat into a chat-first app. The chat now fills the **full content width** (no narrow reading column). The **left rail is collapsible** (chevron to hide, floating `☰` to reopen, persisted in `localStorage`; the class toggles on `<body>` so the rail, the floating button, and the content padding all respond) and is now the navigator: **"+ New chat"** on top, the **session list** (newest first, each clickable, active one highlighted, ✕ to delete), and a pinned bottom block with the **active profile + model** chip (links to Profiles) and a **Settings** group (Profiles, Keys & secrets). The standalone **Overview** and **Sessions** tabs are gone — `/` now 307-redirects to `/chat`, and sessions live in the rail via `panel.sidebar_context()` rendered on every page. **Opening an old session repaints its transcript**: `WebChatManager.history_messages()` serializes the resumed `message_history` (user prompts → bubbles, assistant `TextPart` → markdown bubbles, tool calls → completed chips; system/return/thinking parts skipped) and `GET /chat/history` feeds `chat.js`, which repaints on the `SessionStarted` frame whenever the session id changes. "+ New chat" (`/chat?new=1`) forces a fresh session *before* the stream connects so the previous transcript never flashes; the `new` flag stays out of the persistent EventSource URL so reconnects can't spawn sessions. **SSE connection hygiene:** because each `/chat` (re)load opens a persistent `/chat/stream` EventSource that holds one of the browser's ~6 HTTP/1.1 connections-per-host — and browsers can keep the prior page (with its open SSE) in the back/forward cache — `chat.js` keeps exactly one EventSource and closes it on `pagehide`/`beforeunload` (reconnecting on bfcache `pageshow`). Without this, a few session clicks saturated the pool and froze the UI ("loads and loads"); verified fixed (6 session navigations leave fetches <35 ms). **Polish pass:** the cramped bottom block (duplicate-looking profile/model chip + Profiles/Keys links + "project · local-first · single-user" subtexts) was replaced with a single **⚙ Settings** entry that opens a new `/settings` landing page (shows active profile/model/scope once, links out to Profiles and Keys, and flags per-session model/profile switching as a future CLI-parity feature). Deleting a session from the rail now redirects to `/chat` (home) instead of the removed `/sessions` page (which 404'd).

**Environment panels + theme pass (done, 2026-05-31):** inspired by a Material-dark reference mockup (`ref/chat_page_ref.html`) — adopted the *aesthetic* (warm near-black surfaces, gold primary, slim scrollbars, rounded bubbles) within the existing **zero-dep, offline** CSS (no Tailwind CDN / Google Fonts / remote images). Added an **Environment** block to the chat activity sidebar with three minimal read-only panels — **A2A peers**, **MCP servers** (transport, enabled, `hitl`), **skills** (name + source) — sourced from the live `a2a_capability` / `mcp_capability` / `skills_capability` via a new `WebChatManager.environment()` + `GET /chat/environment`, fetched once per session (not polled). Palette refresh lives in `static/jac.css` `:root` so it cascades to every panel page.

**Fix — A2A peer auth discriminator survives profile serialization (2026-05-31).** Persisting a profile whose `a2a.peers` carried an auth block (`BearerAuth`/`ApiKeyAuth`/`OAuth2ClientCredentialsAuth`) made it unloadable: `model_dump(exclude_defaults=True)` — used by `add_or_update_profile` and `profile_to_yaml` — dropped the auth `type:` discriminator because it equals its `Literal` default, leaving `auth: {token: …}` with no tag, which then failed `PeerAuth` discrimination on reload (`union_tag_not_found` → `JacConfigError`). This hit `jac profiles edit`, the web profile-YAML save, and any round-trip. Fixed at the model layer: a shared `_AuthStrategy` base with a `model_serializer(mode="wrap")` re-injects `type` whenever a parent dump dropped it, so every path round-trips while other `exclude_defaults` trimming (e.g. oauth2 `scope=""`) is unchanged. The web `_edit_default_profile_a2a` raw-subtree workaround stays (it scopes the write to just the `a2a` block) but no longer carries the burden of the discriminator bug. Regression tests in [`tests/test_profiles.py`](../tests/test_profiles.py) cover all three auth types, the on-disk YAML, and the `profile_to_yaml` editor view.

**Remaining: visual/theming polish** — further animations, imagery, richer minion aesthetic (the natural place to introduce a heavier reactive frontend if it earns its keep).
