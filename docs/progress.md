...
- **Current active work:** **Web surface (D48) — Slice 1 (local-first control panel) landed 2026-05-31** under `src/jac/web/` (`jac web serve`); see the "Web surface" section below. Slice 2 (streaming chat + HITL) is next. **Phase G — Plan Mode (D23) landed 2026-05-30 in v0.7.0** alongside Accept-Edits mode + a compaction-control pass. **Phase F (MCP loader + tool search) landed 2026-05-29.** Also queued: Phase H (A2A 4.e + broader tests). YOLO mode is still v2 (needs `pydantic-monty` sandboxing per D43).
